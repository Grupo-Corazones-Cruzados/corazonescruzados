...
| Rail de filtro | `components/ui/FilterRail` | **único** rail de filtro: icono + label + conteo, activo con barra izq. accent. Admite `hint` (2ª línea) y `sections` (grupos con encabezado y separador). Ancho 220px **por defecto**, y `className` lo sustituye: cuando el rail comparte columna con un botón de ancho completo (Admin → Soluciones), va `className="lg:w-full"` para que la tarjeta no salga más estrecha que el botón de encima (Fernando, 2026-08-23). **Un rail nuevo se hace con este componente, nunca copiando el marcado** — estaba duplicado en 12 pantallas y por eso unas y otras se veían distintas (migradas todas el 2026-07-26) |
...
- **2026-09-21 · Logo girando · ADOPTADO como definición única:** `components/ui/LogoGirando.tsx`
  ...
  con su `<Image>` inline con la misma animación: migrar cuando se toque. `BrandLoader` queda
  solo para cargas de verdad.
