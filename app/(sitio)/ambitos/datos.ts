...
import { listarAmbitos, contenidoDeTalento, type ContenidoDeTalento } from '@/lib/ambitos';
import type { AmbitoConContenido } from '@/components/sitio/AmbitosExplorador';
 ...
 * buena y reintenta en la siguiente revalidación. Devolver «no hay ámbitos» cuando lo que hay
 * es una base caída es el fallo que ya costó una investigación entera.
 */
export async function cargarAmbitosConContenido(): Promise<AmbitoConContenido[]> {
  const cargar = async (): Promise<AmbitoConContenido[]> => {
    const ambitos = await listarAmbitos();

    // El contenido se consulta UNA vez por talento. Un talento pertenece a un solo ámbito
