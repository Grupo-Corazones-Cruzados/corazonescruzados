...
/**
 * LA PÁGINA DEL ENLACE DE PAGO — canal 3, para el cliente que NO tiene cuenta.
 *
 ...
 *
 * Vive en `(sitio)`, así que hereda el marco público —cabecera oscura, cuerpo claro, pie
 * oscuro— y es **hermana deliberada de `/proyecto/[id]`**: quien recibe las dos por correo
 * tiene que reconocer que son de la misma casa. Por eso reusa `Contenedor` y `Tarjeta` y
 * repite su forma de listar las etapas, en vez de inventarse un lenguaje propio.
 *
...
export default function PaginaPagar() {
  ...
  useEffect(() => {
    fetch(`/api/pagos/etapa?link=${encodeURIComponent(String(token))}`)
      .then(r => r.json())
  ...
  return (
    <Contenedor className="py-14 sm:py-20">
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] items-start">
        ...
        {/* ── El pago ── */}
        <Tarjeta>
          <FormularioPago datos={datos} link={String(token)} />
        </Tarjeta>
      </div>
    </Contenedor>
  );
