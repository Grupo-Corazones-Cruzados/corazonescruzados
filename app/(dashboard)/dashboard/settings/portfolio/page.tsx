...
import { FolderKanban, Package, Workflow, Plus, Pencil, Trash2, UploadCloud, X, ChevronLeft, Image as ImageIcon, Users } from 'lucide-react';
...
export default function PortfolioPage() {
  ...
  const firstImage = (item: any): string | null => (Array.isArray(item.images) && item.images[0]) || item.image_url || null;
  ...
  /* ── Cards ── */
  const OwnCard = ({ item }: { item: any }) => {
    const img = firstImage(item); const count = imageCount(item);
    return (
      <div className="flex flex-col bg-digi-card border border-digi-border rounded-xl overflow-hidden shadow-sm">
        <div className="relative aspect-[16/9] bg-digi-darker overflow-hidden">
          ...
          {count > 0 && (
            <button onClick={(e) => { e.stopPropagation(); openGalleryFor(item.images?.length ? item.images : [item.image_url], item.title); }}
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          {Array.isArray(item.tags) && item.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">{item.tags.slice(0, 3).map((t: string) => <PixelBadge key={t}>{t}</PixelBadge>)}</div>
          )}
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-digi-border/60">
            <button onClick={() => openEdit(item)} className={`${BTN_SECONDARY} flex-1`}><Pencil className="w-3.5 h-3.5" /> Editar</button>
            <button onClick={() => handleDelete(item.id)} title="Eliminar"
              className="w-9 h-9 flex items-center justify-center rounded-md border border-digi-border text-digi-muted hover:text-red-600 hover:border-red-300 hover:bg-red-50 transition-colors shrink-0"><Trash2 className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
    );
  ...
  const TeamCard = ({ p }: { p: any }) => {
    const img = firstImage(p); const count = p.images?.length || 0; const team = p.team || [];
    return (
      <div className="flex flex-col bg-digi-card border border-digi-border rounded-xl overflow-hidden shadow-sm">
        <div className="relative aspect-[16/9] bg-digi-darker overflow-hidden">
          {img ? (
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <div className="flex items-center justify-between gap-2 mt-3 pt-3 border-t border-digi-border/60">
                ...
                <>
                  <div className="flex items-center pl-1">
                    {team.slice(0, 3).map((m: any, i: number) => (
                      m.photo_url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img key={i} src={m.photo_url} alt={m.name} title={m.name} className="w-5 h-5 rounded-full object-cover border border-digi-border -ml-1 first:ml-0" />
                      ) : (
                        <div key={i} title={m.name} className="w-5 h-5 rounded-full bg-accent-light border border-accent/20 flex items-center justify-center -ml-1 first:ml-0"><span className="text-[10px] font-semibold text-accent">{m.name?.charAt(0)}</span></div>
                      )
                    ))}
                  </div>
                  <span className="text-[11px] text-digi-muted truncate" style={mf}>{team.length} miembro{team.length === 1 ? '' : 's'}</span>
                </>
              ) : <span className="inline-flex items-center gap-1 text-[11px] text-digi-muted/60" style={mf}><Users className="w-3.5 h-3.5" /> —</span>}
            </div>
            <button onClick={() => openTeamEdit(p)} className={`${BTN_SECONDARY} shrink-0`}><Pencil className="w-3.5 h-3.5" /> Editar</button>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex gap-2 flex-wrap mt-1">
          {images.filter(Boolean).map((img, i) => (
            <div key={i} className="relative w-16 h-16 rounded-md border border-digi-border overflow-hidden group">
              ...
              <button type="button" onClick={() => removeImage(i, setImages, images)}
                className="absolute top-0.5 right-0.5 w-5 h-5 rounded-full bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-3 h-3" /></button>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      ...
      {/* toolbar: categorías (segmented) + Nuevo */}
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">{items.map((item) => <OwnCard key={item.id} item={item} />)}</div>
      )}
      ...
      {tab === 'project' && teamProjects.length > 0 && (
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">{teamProjects.map((p) => <TeamCard key={p.id} p={p} />)}</div>
        </div>
      )}
