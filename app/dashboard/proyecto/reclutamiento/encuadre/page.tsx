...
export default function EncuadrePage() {
  ...
  const determineRole = useCallback(async () => {
    try {
      // Try fetching postulaciones - if 403, user is a client
      ...
      if (res.ok) {
        ...
        // We need a different check for client
      } else {
        setRol("cliente");
      }
    } catch {
  ...
  const checkClientStatus = useCallback(async () => {
    try {
      // Try to post with empty to check status (will fail but shows restriction)
      const res = await fetch("/api/reclutamiento/postulaciones", {
        method: "POST",
