...
<div style="max-width:600px;margin:0 auto;background:#ffffff;">
  ...
  <div style="padding:30px 40px;">
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
      ...
      <tr><td style="padding:10px 16px;color:#666;font-size:13px;${inv.currency && inv.currency !== 'USD' ? 'border-bottom:1px solid #f0f0f0;' : ''}"><strong>Valor Total:</strong></td><td style="padding:10px 16px;font-size:18px;font-weight:bold;color:#1a1a2e;${inv.currency && inv.currency !== 'USD' ? 'border-bottom:1px solid #f0f0f0;' : ''}">${Number(inv.total).toFixed(2)} ${inv.currency || 'USD'}</td></tr>
      ${inv.currency && inv.currency !== 'USD' ? `<tr><td style="padding:10px 16px;color:#666;font-size:13px;"><strong>Ref. USD:</strong></td><td style="padding:10px 16px;font-size:13px;color:#888;">$${(Number(inv.total) / Number(inv.exchange_rate)).toFixed(2)} USD (tasa: ${Number(inv.exchange_rate)})</td></tr>` : ''}
    </table>
