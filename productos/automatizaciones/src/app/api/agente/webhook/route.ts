/**
 * Webhook de la Cloud API de Meta — versión del PRODUCTO.
 *
 * ⚠️ Mientras Meta siga apuntando a `app.grupocc.org/api/agente/webhook`, esta ruta NO
 * recibe nada: existe para que el cambio de guardia se pueda hacer sin ventana muerta.
 * La secuencia es levantar esto, comprobarlo con `hub.verify_token`, correr la mudanza de
 * datos una última vez y ENTONCES cambiar la URL en Meta.
 *
...
export async function POST(req: Request) {
  ...
  if (!firmaValida(crudo, req.headers.get('x-hub-signature-256'), process.env.WHATSAPP_APP_SECRET)) {
    ...
    try {
      await pool.query(
        `INSERT INTO eventos_webhook (firma_valida, payload) VALUES (false, $1)`,
  ...
  try {
    ...
    await pool.query(
      // El inquilino sale del canal cuando lo hay; si el número no es de nadie nuestro
      // queda nulo, que es lo correcto: la traza vale igual para diagnosticar un alta a
      // medias, y por eso esta columna SÍ admite nulos.
      `INSERT INTO eventos_webhook (inquilino_id, canal_id, firma_valida, payload)
       VALUES ($1, $2, true, $3)`,
      [canal?.inquilino_id ?? null, canal?.id ?? null, crudo.slice(0, 100_000)],
    );
