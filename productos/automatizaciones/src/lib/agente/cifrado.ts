...
export function contextoCanal(canalId: number, campo: CampoSecreto): string {
  /**
   * ⚠️⚠️ ESTA CADENA NO SE RENOMBRA. NUNCA.
   *
   * Dice «agente_canales» —el nombre que la tabla tenía en la plataforma— y aquí la tabla
   * se llama `canales`. Parece un descuido del porte y es justo lo contrario: esta cadena
   * es el DATO ASOCIADO AUTENTICADO con el que se cifraron los secretos que se migraron.
   * Si se «corrige» a `canales`, el descifrado de los tokens de WhatsApp que ya existen
   * falla con un error de autenticación, el agente no puede enviar y el cliente se queda
   * mudo sin que nada explique por qué.
   *
   * Por el mismo motivo la mudanza CONSERVÓ los identificadores de canal (11 y 33): el id
   * también entra aquí. Cambiar cualquiera de las dos cosas rompe lo ya cifrado.
   */
