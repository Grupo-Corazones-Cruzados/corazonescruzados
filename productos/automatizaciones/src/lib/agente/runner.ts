...
async function escalar(conversacionId: number, motivo: string, _aviso: string) {
  await pool.query(
    `UPDATE conversaciones
        SET bot_activo = false, motivo_escalado = $2, tomada_en = NOW(), actualizado_en = NOW()
      WHERE id = $1`,
...
async function promptsDelCanal(canalId: number): Promise<Record<string, string>> {
  const { rows } = await pool.query(
    `SELECT tipo, contenido FROM prompts WHERE canal_id = $1 AND activo`,
    [canalId],
  );
...
async function conocimientoDelCanal(canalId: number): Promise<BloqueConocimiento[]> {
  const { rows } = await pool.query(
    `SELECT clave, titulo, contenido, orden, activo
       FROM conocimiento WHERE canal_id = $1 ORDER BY orden, clave`,
    [canalId],
  );
...
async function registrarUso(conversacionId: number, uso: any, herramienta?: string) {
  await pool.query(
    `INSERT INTO uso_modelo
       (inquilino_id, conversacion_id, modelo, tokens_entrada, tokens_salida,
        tokens_cache_escritura, tokens_cache_lectura, herramienta, duracion_ms)
     SELECT cv.inquilino_id, cv.id, $2,$3,$4,$5,$6,$7,$8
