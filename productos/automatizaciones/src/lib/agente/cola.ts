...
export async function encolar(conversacionId: number, debounceSegundos: number): Promise<void> {
  await pool.query(
    `INSERT INTO cola (inquilino_id, conversacion_id, ejecutar_en)
     SELECT cv.inquilino_id, cv.id, NOW() + ($2 || ' seconds')::interval
...
export async function reclamar(limite = 5): Promise<Trabajo[]> {
  const { rows } = await pool.query<Trabajo>(
    `UPDATE cola c
        SET estado = 'procesando', reclamado_en = NOW(), intentos = c.intentos + 1, actualizado_en = NOW()
      WHERE c.id IN (
        SELECT id FROM cola
...
export async function terminar(trabajoId: number): Promise<void> {
  await pool.query(
    `UPDATE cola SET estado = 'hecho', error = NULL, actualizado_en = NOW() WHERE id = $1`,
    [trabajoId],
  );
}
...
export async function fallar(trabajo: Trabajo, mensaje: string): Promise<'reintenta' | 'agotado'> {
  ...
  if (agotado) {
    await pool.query(
      `UPDATE cola SET estado = 'error', error = $1, actualizado_en = NOW() WHERE id = $2`,
  ...
  await pool.query(
    `UPDATE cola
        SET estado = 'pendiente', error = $1, ejecutar_en = NOW() + ($2 || ' seconds')::interval, actualizado_en = NOW()
      WHERE id = $3`,
...
export async function recuperarColgados(): Promise<number> {
  const { rowCount } = await pool.query(
    `UPDATE cola
        SET estado = 'pendiente', ejecutar_en = NOW(), actualizado_en = NOW()
