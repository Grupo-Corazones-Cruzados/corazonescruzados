...
export async function sincronizarPlantillas(canal: any): Promise<{ total: number; nuevas: number }> {
  ...
  for (const p of deMeta) {
    ...
    const { rowCount } = await pool.query(
      `INSERT INTO plantillas_agente
         (inquilino_id, canal_id, meta_id, nombre, idioma, categoria, estado, motivo_rechazo,
          encabezado, cuerpo, pie, sincronizado_en, actualizado_en)
       SELECT c.inquilino_id, c.id, $2,$3,$4,$5,$6,$7,$8,$9,$10, NOW(), NOW()
       ...
       ON CONFLICT (canal_id, nombre, idioma) DO UPDATE
          SET meta_id = EXCLUDED.meta_id,
              ...
              sincronizado_en = NOW(),
              actualizado_en = NOW()
        WHERE plantillas_agente.meta_id IS DISTINCT FROM EXCLUDED.meta_id
           OR plantillas_agente.estado  IS DISTINCT FROM EXCLUDED.estado
           OR plantillas_agente.cuerpo  IS DISTINCT FROM EXCLUDED.cuerpo`,
  ...
  await pool.query(
    `UPDATE plantillas_agente
        SET estado = 'ausente', actualizado_en = NOW()
...
export async function enviarAListado(
  ...
  const { rows: [envio] } = await pool.query(
    `INSERT INTO envios (inquilino_id, canal_id, plantilla_id, lista_id, total, lanzado_por_id)
       SELECT c.inquilino_id, c.id, $2,$3,$4,$5 FROM canales c WHERE c.id = $1
       RETURNING id`,
  ...
  for (const c of contactos) {
    ...
    await pool.query(
      `UPDATE envios SET enviados = $2, fallidos = $3 WHERE id = $1`,
...
async function registrarSaliente(
  ...
  const { rows: [contacto] } = await pool.query(
    `INSERT INTO contactos (inquilino_id, canal_id, wa_id, nombre_perfil, actualizado_en)
     ...
     ON CONFLICT (canal_id, wa_id) DO UPDATE SET nombre_perfil = COALESCE(contactos.nombre_perfil, EXCLUDED.nombre_perfil)
     RETURNING id`,
    [canalId, waId, nombre],
  );
  ...
  await pool.query(
    `INSERT INTO mensajes
       (inquilino_id, conversacion_id, direccion, wa_message_id, tipo, texto, herramienta,
        enviado_ok, error_envio, envio_id)
     SELECT cv.inquilino_id, cv.id, 'SALIENTE', $2, 'template', $3, 'plantilla', $4, $5, $6
