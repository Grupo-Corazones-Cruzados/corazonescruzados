...
export async function ingerir(canalId: number, m: MensajeEntrante): Promise<ResultadoIngesta> {
  ...
  try {
    ...
    const { rows: [contacto] } = await cliente.query(
      // ⭐ EL INQUILINO SALE DE LA FILA PADRE, NO DE JAVASCRIPT. Es lo que hace
      // IMPOSIBLE escribir una fila en el inquilino equivocado: el contacto es del
      // inquilino de su canal, y eso lo decide la base, no quien llama.
      `INSERT INTO contactos (inquilino_id, canal_id, wa_id, nombre_perfil, actualizado_en)
       SELECT c.inquilino_id, c.id, $2, $3, NOW() FROM canales c WHERE c.id = $1
    ...
    const { rows: [conversacion] } = await cliente.query(
      `INSERT INTO conversaciones (inquilino_id, canal_id, contacto_id, ultimo_mensaje_en, actualizado_en)
       SELECT c.inquilino_id, c.id, $2, NOW(), NOW() FROM canales c WHERE c.id = $1
    ...
    const { rows: insertado } = await cliente.query(
      `INSERT INTO mensajes
         (inquilino_id, conversacion_id, direccion, wa_message_id, tipo, texto, payload, ubicacion_lat, ubicacion_lng)
       SELECT cv.inquilino_id, cv.id, 'ENTRANTE', $2, $3, $4, $5::jsonb, $6, $7
...
export async function registrarSaliente(opciones: {
  ...
  const { rows: [fila] } = await pool.query(
    `INSERT INTO mensajes
       (inquilino_id, conversacion_id, direccion, wa_message_id, tipo, texto, herramienta, motivo, enviado_ok, error_envio)
     SELECT cv.inquilino_id, cv.id, 'SALIENTE', $2, 'text', $3, $4, $5, $6, $7
       FROM conversaciones cv WHERE cv.id = $1
     RETURNING id`,
    [
  ...
  await pool.query(
    `UPDATE conversaciones SET ultimo_mensaje_en = NOW(), actualizado_en = NOW() WHERE id = $1`,
    [opciones.conversacionId],
  );
  return fila.id;
}
...
export async function tomarConversacion(
  conversacionId: number,
  /** El usuario DEL PRODUCTO (entero), no el uuid de la plataforma. */
  usuarioId: number | null,
  motivo?: string,
): Promise<void> {
  await pool.query(
    `UPDATE conversaciones
        SET bot_activo = $2, tomada_por_id = $3, tomada_en = CASE WHEN $3 IS NULL THEN NULL ELSE NOW() END,
            motivo_escalado = $4, actualizado_en = NOW()
      WHERE id = $1`,
...
async function contactoYConversacion(canalId: number, waId: string) {
  const { rows: [contacto] } = await pool.query(
    `INSERT INTO contactos (inquilino_id, canal_id, wa_id, actualizado_en)
     SELECT c.inquilino_id, c.id, $2, NOW() FROM canales c WHERE c.id = $1
...
export async function ingerirEco(canalId: number, e: EcoDelEquipo): Promise<{ esNuevo: boolean; conversacionId: number }> {
  ...
  try {
    ...
    const { rows: [contacto] } = await cliente.query(
      `INSERT INTO contactos (inquilino_id, canal_id, wa_id, actualizado_en)
       SELECT c.inquilino_id, c.id, $2, NOW() FROM canales c WHERE c.id = $1
       ON CONFLICT (canal_id, wa_id) DO UPDATE SET actualizado_en = NOW() RETURNING id`,
      [canalId, e.waId],
    );
    const { rows: [conv] } = await cliente.query(
      `INSERT INTO conversaciones (inquilino_id, canal_id, contacto_id, ultimo_mensaje_en, actualizado_en)
       SELECT c.inquilino_id, c.id, $2, NOW(), NOW() FROM canales c WHERE c.id = $1
       ON CONFLICT (canal_id, contacto_id) DO UPDATE SET ultimo_mensaje_en = NOW(), actualizado_en = NOW()
       RETURNING id`,
      [canalId, contacto.id],
    );
    ...
    const { rows: insertado } = await cliente.query(
      `INSERT INTO mensajes
         (inquilino_id, conversacion_id, direccion, wa_message_id, tipo, texto, payload, herramienta, enviado_ok)
       SELECT cv.inquilino_id, cv.id, 'SALIENTE', $2, $3, $4, $5::jsonb, 'equipo', true
         FROM conversaciones cv WHERE cv.id = $1
       ON CONFLICT (wa_message_id) WHERE wa_message_id IS NOT NULL DO NOTHING
       RETURNING id`,
...
export async function guardarContactosDeAgenda(canalId: number, contactos: ContactoDeAgenda[]): Promise<number> {
  ...
  for (const c of masNuevo.values()) {
    const { rowCount } = await pool.query(
      `INSERT INTO contactos (inquilino_id, canal_id, wa_id, nombre_agenda, actualizado_en)
       SELECT c.inquilino_id, c.id, $2, $3, NOW() FROM canales c WHERE c.id = $1
       ON CONFLICT (canal_id, wa_id) DO UPDATE
         SET nombre_agenda = EXCLUDED.nombre_agenda, actualizado_en = NOW()`,
