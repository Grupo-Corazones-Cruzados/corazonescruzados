...
import { exigirSesionDelNegocio } from '@/lib/inquilino';
...
const MENSAJES = {
  vencido: {
    ...
    texto:
      'El acceso a la aplicación se reanuda en cuanto se registre el pago del mes. Los datos del negocio siguen guardados y no se pierde nada.',
  },
  'sin-pago': {
    icono: CreditCard,
...
export default async function PaginaSuscripcion({ params }: { params: Promise<{ negocio: string }> }) {
  const { negocio } = await params;
  const { inquilino, acceso } = await exigirSesionDelNegocio(negocio);
  if (!inquilino) notFound();

  // Si la mensualidad está al día esta pantalla no tiene nada que decir.
  if (acceso === 'ok') redirect(`/${negocio}`);
      ...
      <div className="tarjeta w-full max-w-lg p-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-tenue">Gestión de Catering</p>
          </div>
        </div>
