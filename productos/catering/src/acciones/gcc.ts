...
const RESERVADOS = new Set(['gcc', 'api', 'admin', 'acceso', '_next', 'panel', 'clientes', 'servicios', 'menus', 'etiquetas', 'rutas', 'registro', 'catering']);
...
const Alta = z.object({
  ...
  nombre: z.string().trim().min(2, 'Escribe el nombre del negocio.'),
...
/**
 * Alta de un negocio. Crea de una vez el inquilino, su suscripción y la
 * primera cuenta de administrador: un negocio sin nadie que pueda entrar no está
...
export async function crearInquilino(datos: FormData): Promise<ResultadoGcc> {
  ...
  // de la propia aplicación o el negocio quedaría inaccesible.
  ...
  if (repetido) return { ok: false, error: `Ya hay un negocio con el código «${d.slug}».` };
  ...
  revalidatePath('/gcc');
  return { ok: true, clave, slug: d.slug, mensaje: `Negocio «${d.nombre}» creado.` };
}
...
export async function registrarPago(datos: FormData): Promise<ResultadoGcc> {
  ...
  if (!suscripcion) return { ok: false, error: 'Ese negocio no tiene suscripción.' };
...
export async function cambiarEstadoInquilino(
  ...
  return {
    ok: true,
    mensaje: estado === 'SUSPENDIDO' ? 'Negocio suspendido.' : 'Negocio reactivado.',
  };
}

/**
 * Poner o quitar el modo escaparate. Es del equipo GCC, no del negocio: si el propio
 * negocio pudiera quitárselo, no sería una garantía, sería una sugerencia.
 */
export async function cambiarSoloLectura(
  ...
  return {
    ok: true,
    mensaje: soloLectura
      ? 'Negocio en modo escaparate: se puede ver, no se puede guardar.'
 ...
 * Grupo Corazones Cruzados no paga mensualidad ni tiene topes ni purga. Lo decide
 * el equipo GCC, nunca el propio inquilino.
 */
...
export async function guardarPlan(id: number | null, datos: FormData): Promise<ResultadoGcc> {
  ...
  const comun = {
    ...
    mesesRetencion: tope('mesesRetencion'),
...
export async function restablecerClaveAdmin(inquilinoId: number): Promise<ResultadoGcc> {
  ...
  if (!admin) return { ok: false, error: 'Ese negocio no tiene administrador activo.' };
