...
import type { RolUsuario } from '@/generated/prisma/enums';import {
...
export async function entrar(slug: string, datos: FormData): Promise<ResultadoAcceso> {
  ...
  redirect(evaluarAcceso(inquilino) === 'ok' ? `/${inquilino.slug}/panel` : `/${inquilino.slug}/suscripcion`);
}
...
export async function entrarConCodigo(slug: string, codigo: string): Promise<ResultadoAcceso> {
  ...
  redirect(evaluarAcceso(c.inquilino) === 'ok' ? `/${c.inquilino.slug}/panel` : `/${c.inquilino.slug}/suscripcion`);
}
