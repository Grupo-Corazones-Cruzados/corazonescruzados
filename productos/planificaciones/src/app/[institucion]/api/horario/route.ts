...
import { DIAS, ETIQUETA_DIA, HORAS, SIN_CLASE, etiquetaHora, horarioDe, materiasDelDocente } from '@/lib/horario';

export const dynamic = 'force-dynamic';

/**
 * El Excel del horario: hoja «Horario» con las ocho horas (filas) y los cinco
 * días (columnas), cada celda con una lista desplegable de las materias que el
...
export async function GET(_p: Request, { params }: { params: Promise<{ institucion: string }> }) {
  ...
  HORAS.forEach((h, i) => {
    hoja.getCell(i + 2, 1).value = etiquetaHora(h);
    hoja.getCell(i + 2, 1).font = { bold: true };
    DIAS.forEach((d, j) => {
      const c = hoja.getCell(i + 2, j + 2);
      const puesto = actual.find((x) => x.dia === d && x.hora === h);
  ...
  hoja.getCell(11, 1).value = 'Elige en cada celda una de tus materias (con su grado) o «Sin clase». Deja vacías las horas que no apliquen. Luego importa este archivo desde tu perfil.';
  hoja.getCell(11, 1).font = { italic: true, color: { argb: 'FF616161' } };
