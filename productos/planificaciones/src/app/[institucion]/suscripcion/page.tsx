...
import { exigirSesionDeLaInstitucion } from '@/lib/inquilino';
...
const MENSAJES = {
  vencido: {
    ...
    texto:
      'El acceso a la aplicación se reanuda en cuanto se registre el pago del mes. Las planificaciones siguen guardadas y no se pierde nada.',
  },
  'sin-pago': {
    icono: CreditCard,
...
export default async function PaginaSuscripcion({ params }: { params: Promise<{ institucion: string }> }) {
  const { institucion } = await params;
  const { inquilino, acceso } = await exigirSesionDeLaInstitucion(institucion);
  if (!inquilino) notFound();

  // Si la mensualidad está al día esta pantalla no tiene nada que decir.
  if (acceso === 'ok') redirect(`/${institucion}`);
    ...
    >
      <div className="tarjeta w-full max-w-lg p-8">
        <div className="mb-6 flex items-center gap-3">
          <LogoNegocio nombre={inquilino.nombre} logoUrl={inquilino.logoUrl} tamano={40} />
          <div className="min-w-0">
            <p className="truncate text-[14px] font-semibold text-texto">{inquilino.nombre}</p>
            <p className="text-[11px] text-tenue">Planificación de Clases</p>
          </div>
        </div>
