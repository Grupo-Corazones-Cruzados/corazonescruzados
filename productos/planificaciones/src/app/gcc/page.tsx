...
export default async function PaginaGcc() {
  ...
  const [inquilinos, planes] = await Promise.all([
    prisma.inquilino.findMany({
      ...
      include: {
        ...
        _count: { select: { usuarios: true, planificaciones: true, semanas: true } },
  ...
  const filas: InquilinoGcc[] = inquilinos.map((i) => ({
    ...
    planificaciones: i._count.planificaciones,
    semanas: i._count.semanas,
    pagos: i.pagos.map((p) => ({
      periodo: p.periodo,
  ...
  const listaPlanes: PlanGcc[] = planes.map((p) => ({
    ...
    maxGeneracionesSemana: p.maxGeneracionesSemana,
    mesesRetencion: p.mesesRetencion,
