...
const RESERVADOS = new Set(['gcc', 'api', 'admin', 'acceso', '_next', 'panel', 'planificaciones', 'perfil', 'usuarios', 'configuracion', 'suscripcion']);
...
const Alta = z.object({
  ...
  nombre: z.string().trim().min(2, 'Escribe el nombre de la institución.'),
...
/**
 * Alta de una institución. Crea de una vez el inquilino, su suscripción y la
 * primera cuenta de administrador: una institución sin nadie que pueda entrar no está
...
export async function crearInquilino(datos: FormData): Promise<ResultadoGcc> {
  ...
  // de la propia aplicación o la institución quedaría inaccesible.
  ...
  if (repetido) return { ok: false, error: `Ya hay una institución con el código «${d.slug}».` };
  ...
  revalidatePath('/gcc');
  return { ok: true, clave, slug: d.slug, mensaje: `Institución «${d.nombre}» creada.` };
}
...
export async function registrarPago(datos: FormData): Promise<ResultadoGcc> {
  ...
  if (!suscripcion) return { ok: false, error: 'Esa institución no tiene suscripción.' };
...
export async function cambiarEstadoInquilino(
  ...
  return {
    ok: true,
    mensaje: estado === 'SUSPENDIDO' ? 'Institución suspendida.' : 'Institución reactivada.',
  };
}

/**
 * Poner o quitar el modo escaparate. Es del equipo GCC, no de la institución: si la propia
 * institución pudiera quitárselo, no sería una garantía, sería una sugerencia.
 */
...
export async function guardarPlan(id: number | null, datos: FormData): Promise<ResultadoGcc> {
  ...
  const comun = {
    ...
    maxGeneracionesSemana: tope('maxGeneracionesSemana'),
...
export async function restablecerClaveAdmin(inquilinoId: number): Promise<ResultadoGcc> {
  ...
  if (!admin) return { ok: false, error: 'Esa institución no tiene administrador activo.' };
