...
export async function crearGrado(slug: string, datos: FormData): Promise<Resultado> {
  ...
  const g = await prisma.grado.create({ data: { inquilinoId: permiso.ctx.inquilino.id, nombre: nombre.data, orden: (ultimo._max.orden ?? -1) + 1 } });
...
export async function renombrarGrado(slug: string, id: number, datos: FormData): Promise<Resultado> {
  ...
  await prisma.grado.update({ where: { id }, data: { nombre: nombre.data } });
  revalidatePath(`/${slug}/unidades`);
  return { ok: true, id };
}
