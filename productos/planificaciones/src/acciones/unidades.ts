...
/** El nivel del grado (Preparatoria por defecto): el de sus destrezas y planificaciones. */
const leerNivel = (v: FormDataEntryValue | null, porDefecto: Nivel = 'PREPARATORIA'): Nivel => ((NIVELES as string[]).includes(String(v)) ? (String(v) as Nivel) : porDefecto);
...
export async function crearGrado(slug: string, datos: FormData): Promise<Resultado> {
  ...
  const nivel = leerNivel(datos.get('nivel'));
  const g = await prisma.grado.create({ data: { inquilinoId: permiso.ctx.inquilino.id, nombre: nombre.data, nivel, color: colorAleatorio(usados), orden: (ultimo._max.orden ?? -1) + 1 } });
...
export async function renombrarGrado(slug: string, id: number, datos: FormData): Promise<Resultado> {
  ...
  await prisma.grado.update({ where: { id }, data: { nombre: nombre.data, nivel: leerNivel(datos.get('nivel'), g.nivel), ...(esColorDeGrado(color) ? { color } : {}) } });
  revalidatePath(`/${slug}/unidades`);
  return { ok: true, id };
}
...
export async function crearMateria(slug: string, gradoId: number, datos: FormData): Promise<Resultado> {
  ...
  const m = await prisma.materiaGrado.create({
    ...
    include: { grado: { select: { nivel: true } } },
  });
  // Nace con las destrezas del catálogo de ese nombre y nivel; el administrador las completa aquí.
  await sembrarDestrezasDeMateria({ materiaGradoId: m.id, inquilinoId: permiso.ctx.inquilino.id, nivel: m.grado.nivel, materia: m.nombre });
  revalidatePath(`/${slug}/unidades`);
