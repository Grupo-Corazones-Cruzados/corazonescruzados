...
export default function MemberLoginModal({
  ...
  const loginWithPasskey = async () => {
    ...
    try {
      ...
      if (!begin.ok) {
        // 403 = la cuenta de este dispositivo no es miembro/admin.
        if (begin.status === 403) {
          setError(opts?.error ?? 'Esta cuenta no es de miembro.');
          return;
        }
        setError(
  ...
  // Registro de passkey (la sesión ya quedó activa tras el login con código).
  const registerPasskey = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Error de passkey';
      // Si el usuario cancela el prompt, entra igual (sin passkey).
