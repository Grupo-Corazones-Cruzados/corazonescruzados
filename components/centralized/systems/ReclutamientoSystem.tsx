 ...
 *
 * Pestaña "Solicitudes": todas las postulaciones enviadas desde el formulario
 * "Quiero postularme como candidato" de la landing (tabla `candidate_proposals`).
 * Layout: rail (pestañas) · lista de solicitudes · panel grande de detalle.
 * Soporta `?solicitud=<id>` para abrir una solicitud directamente por URL.
 */
export default function ReclutamientoSystem({ isAdmin }: { system: any; isAdmin: boolean }) {
  ...
  // Deep-link ?solicitud=<id>: selecciona esa solicitud al cargar.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 items-start">
      ...
      <aside className="w-full lg:w-[200px] shrink-0 bg-digi-card border border-digi-border rounded-lg p-2">
        ...
        <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-left border-l-2 bg-accent-light border-accent text-accent">
          <Inbox className="w-4 h-4 shrink-0 text-accent" />
          <span className="flex-1 min-w-0 text-[12.5px] font-medium truncate" style={mf}>Solicitudes</span>
          {pendingCount > 0 && (
            <span className="text-[10px] px-1.5 py-0.5 rounded-full tabular-nums bg-accent/15 text-accent">{pendingCount}</span>
          )}
        </button>
      </aside>

      {/* ── Región derecha: lista · detalle ── */}
      <div className="flex-1 min-w-0 w-full grid grid-cols-1 xl:grid-cols-[minmax(0,340px)_minmax(0,1fr)] gap-4 items-start">
          ...
          {loading ? (
            ...
            <div className="bg-digi-card border border-digi-border rounded-lg py-12 text-center">
              ...
              <p className="text-[12px] text-digi-muted mt-0.5" style={mf}>
                {isAdmin ? 'Aún no hay postulaciones de candidatos.' : 'No tienes acceso a las postulaciones.'}
              </p>
            </div>
          ) : (
                  ...
                  <li key={p.id}>
                    <button onClick={() => setSelectedId(String(p.id))}
                      className={`w-full text-left px-3 py-2.5 flex items-start gap-2.5 transition-colors border-l-2 ${
                        active ? 'bg-accent-light border-accent' : 'border-transparent hover:bg-black/[0.02]'
                      }`}>
          ...
          {selected ? (
            <div className="bg-digi-card border border-digi-border rounded-xl shadow-sm overflow-hidden lg:sticky lg:top-4">
              <div className="flex items-start gap-3 p-4 border-b border-digi-border">
                <div className="w-11 h-11 rounded-lg bg-accent-light border border-accent/20 flex items-center justify-center shrink-0">
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-digi-text leading-relaxed whitespace-pre-wrap rounded-lg border border-digi-border bg-digi-darker p-3" style={mf}>
                    {selected.reason || 'Sin motivo indicado.'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <dl className="rounded-lg border border-digi-border divide-y divide-digi-border/60 overflow-hidden">
                    <Row Icon={selected.email_verified ? ShieldCheck : ShieldAlert} label="Correo verificado"
                      value={<PixelBadge variant={selected.email_verified ? 'success' : 'warning'}>{selected.email_verified ? 'Verificado' : 'Sin verificar'}</PixelBadge>} />
                    <Row Icon={Megaphone} label="Acepta comunicaciones"
                      value={<span className="text-[12.5px] text-digi-text" style={mf}>{selected.marketing ? 'Sí' : 'No'}</span>} />
                    <Row Icon={CalendarDays} label="Fecha de postulación"
                      value={<span className="text-[12.5px] text-digi-text tabular-nums" style={mf}>{fmtDate(selected.created_at)}</span>} />
                    {selected.decided_at && (
                      <Row Icon={Check} label="Decidida"
                        value={<span className="text-[12.5px] text-digi-text" style={mf}>{fmtDate(selected.decided_at)}{selected.decided_by ? ` · ${selected.decided_by}` : ''}</span>} />
                    )}
                  </dl>
                </div>
