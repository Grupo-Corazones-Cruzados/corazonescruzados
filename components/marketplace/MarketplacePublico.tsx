...
import { enlaceDeItem, RUTA_MARKETPLACE_PUBLICO } from '@/lib/marketplace/slug';
...
/**
 * El marketplace público. La misma pantalla sirve `/marketplace-publico` y
 * `/marketplace-publico/<slug>`: con `slugInicial` el catálogo abre ya en la pestaña
 * y el registro del enlace (Fernando, 2026-09-21), y al elegir un registro la
 * dirección se actualiza sin recargar, para que copiarla sea compartirlo.
 */
export default function MarketplacePublico({ slugInicial }: { slugInicial?: string }) {
  const { user } = useAuth();
  const [gateOpen, setGateOpen] = useState(false);

  const reflejarEnDireccion = (item: any | null) => {
    if (typeof window === 'undefined') return;
    const destino = item ? enlaceDeItem(item) : RUTA_MARKETPLACE_PUBLICO;
    if (window.location.pathname !== destino) window.history.replaceState(null, '', destino);
  };
  ...
  return (
    <div className="corp min-h-screen flex flex-col">
      ...
      <main className="flex-1 p-4 md:p-6">
        ...
        <MarketplaceCatalog onPrimaryAction={handlePrimaryAction} slugInicial={slugInicial} onSeleccion={reflejarEnDireccion} />
      </main>
